 ...
 * ----------------------------------------------------------------------------
 * Basic program demonstrating a simple IR photogate on the AVR ATMega328p MCU.
 * Includes digital power management, WLAN WI-FI connection,
 * TCP server creation, and MP3 music control. Utilizes:
...
const char *ALERT_STR			= "Gate triggered.\n";

// GLOBAL VARIABLES -----------------------------------------------------------

static volatile unsigned char powerOn;
static unsigned char gateTriggered = 0;
...
void pollDetectors(void)
{
	...
	{
		if (bit_is_set(PINC, IR1))
		{
			gateTriggered = 1;
			CC3000_sendData(ALERT_STR, strlen(ALERT_STR));
		}
	}
}
