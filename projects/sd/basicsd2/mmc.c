...
#define SELECT()	PORTD &= ~_BV(SDCS)	/* CS = L */
#define	DESELECT()	PORTD |=  _BV(SDCS)	/* CS = H */
#define	MMC_SEL		!(PORTD &  _BV(SDCS))	/* CS status (true:CS == L) */
...
/* Send a 0xFF to the MMC and get the received byte */
BYTE rcv_spi (void)
{
	SPDR = 0xFF;
	loop_until_bit_is_set(SPSR, SPIF);
